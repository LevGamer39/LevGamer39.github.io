...
(function(){
  ...
  const commands = {
    ...
    secret() {
      ...
      appendLine('  sudo   - Шуточная команда с отказом в доступе');
    ...
    help() {
      ...
      appendLine('  sudo        - Шуточная команда');
      appendLine('  hack        - Имитация взлома');
      appendLine('  secret      - Секретная информация');
      appendLine('');
      appendLine('Подсказка: используйте Tab для автодополнения команд и путей');
    },
  ...
  if (out.children.length === 0) {
    ...
    appendLine('Используйте Tab для автодополнения команд и путей');
    appendLine('');
  }
